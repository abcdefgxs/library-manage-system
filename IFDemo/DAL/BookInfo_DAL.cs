...
namespace DAL
{
    public class BookInfo_DAL
    {
        ...
        {
            string sql = @"select BookId,BookName,TimeIn,BookTypeName,Author,
            ...
            while (reader.Read())
            {
                ...
                b.BookNumber = reader.GetString(5);
                b.Price = reader.GetString(6);
                b.ISBS = reader.GetString(7);
                list.Add(b);
            }
            reader.Close();
            return list;
        ...
        {
            string sql = @"select BookId,BookName,TimeIn,BookTypeId,Author,BookNumber,Price,ISBS from BookInfo where BookId=@BookId";
            SqlParameter[] sp ={
        ...
        {
            string sql = @"select BookId,BookName,TimeIn,BookTypeName,Author,BookNumber,Price,ISBS from BookInfo inner join BookType on BookType.BookTypeId=BookInfo.BookTypeId";
        ...
        {
            string sql = @"select BookInfo.BookId as 'BookId',BookName,TimeIn,BookTypeName,Author,
                    BookNumber,Price,ISBS from BookInfo
                    inner join BookType on BookType.BookTypeId=BookInfo.BookTypeId
        ...
        {
            string sql = string.Format(@"select BookId,BookName,TimeIn,BookTypeName,Author,
                            BookNumber,Price,ISBS from BookInfo
                            inner join BookType on BookType.BookTypeId=BookInfo.BookTypeId
                            where {0} like '%{1}%'", A, B);
            return DBhelp.Create().ExecuteAdater(sql);
        ...
        public int ExitBookInfo(BookInfo book)
        {
            ...
            Author=@Author,BookNumber=@BookNumber,Price=@Price,ISBS=@ISBS where BookId=@BookId";
            SqlParameter[] sp ={
                                    new SqlParameter("@BookName",book.BookName),
                                    new SqlParameter("@TimeIn",book.TimeIn),
                                    new SqlParameter("@BookTypeId",book.BookTypeId),
